...
export function TrendDashboardPage() {
  ...
  const ruleOptions = useMemo(() => rules.map((r) => ({ value: r.id, label: r.name })), [rules]);
    ...
    <Stack className="ag-trend-dashboard" gap="md" p={0}>
      ...
      <Group gap="md" className="ag-trend-summary">
        <Card withBorder radius="md" p="md" className="ag-card-tier-2 ag-trend-card">
          ...
          <Text fw={700} size="xl">
            {pct(latestSnapshot?.metrics.firstPassApprovalRate)}
          </Text>
        </Card>
        <Card withBorder radius="md" p="md" className="ag-card-tier-2 ag-trend-card">
          <Text size="xs" c="dimmed">
          ...
          <Text fw={700} size="xl">
            {pct(latestSnapshot?.metrics.validatorPassRate)}
          </Text>
        </Card>
        <Card withBorder radius="md" p="md" className="ag-card-tier-2 ag-trend-card">
          <Text size="xs" c="dimmed">
          ...
          <Text fw={700} size="xl">
            {pct(latestSnapshot?.metrics.autoResolvedRate)}
          </Text>
        </Card>
        <Card withBorder radius="md" p="md" className="ag-card-tier-2 ag-trend-card">
          <Text size="xs" c="dimmed">
          ...
          <Text fw={700} size="xl">
            {latestSnapshot?.metrics.escalationCount ?? "—"}
          </Text>
        </Card>
        <Card withBorder radius="md" p="md" className="ag-card-tier-2 ag-trend-card">
          <Text size="xs" c="dimmed">
          ...
          <Text fw={700} size="xl">
            {latestSnapshot?.metrics.validatorGapCount ?? "—"}
          </Text>
        </Card>
      </Group>
      ...
      <Group gap="xs" className="ag-trend-tabs">
        {(["trends", "improvements", "breakers", ...(expertMode ? (["backtest", "gaps"] as const) : [])] as const).map(
          (t) => (
            <Button key={t} size="xs" variant={tab === t ? "filled" : "light"} onClick={() => setTab(t as typeof tab)}>
              {t === "trends"
                ? "Trend History"
                : t === "improvements"
                  ? "Improvement Runs"
      ...
      {tab === "trends" && (
        <Card withBorder radius="md" p="md" className="ag-card-tier-1">
          <Group justify="space-between" mb="sm">
            ...
            <Button size="xs" variant="light" onClick={handleGenerateSnapshot}>
              Generate Weekly Snapshot
            </Button>
          </Group>
          {snapshots.length === 0 ? (
            <Text size="sm" c="dimmed">
              No snapshots yet. Generate one to start tracking trends.
            </Text>
          ) : (
            <ScrollArea>
              <Table className="ag-trend-table" striped highlightOnHover>
                ...
                <Table.Tbody>
                  {snapshots.map((s) => (
                    <Table.Tr key={s.id}>
                      <Table.Td>
                        {new Date(s.period.from).toLocaleDateString()} – {new Date(s.period.to).toLocaleDateString()}
                      </Table.Td>
                      <Table.Td>
                        <Badge variant="light" size="sm">
                          {s.period.granularity}
                        </Badge>
                      </Table.Td>
                      <Table.Td>
                        {s.metrics.totalJobs ?? 0} ({s.metrics.succeededJobs ?? 0}✓ {s.metrics.failedJobs ?? 0}✗)
                      </Table.Td>
      ...
      {tab === "improvements" && (
        <Card withBorder radius="md" p="md" className="ag-card-tier-1">
            ...
            <Card withBorder radius="sm" p="sm" mb="sm" className="ag-card-tier-2">
              <Stack gap="xs">
                <TextInput
                  ...
                  value={newRunName}
                  onChange={(e) => setNewRunName(e.currentTarget.value)}
                />
                <Group gap="xs">
                  <Select
                    size="xs"
                    ...
                    value={newRunCohortMethod}
                    onChange={setNewRunCohortMethod}
                  />
                  {newRunCohortMethod && newRunCohortMethod !== "all" && (
                    <TextInput
                      size="xs"
                      placeholder="Filter value"
                      value={newRunCohortValue}
                      onChange={(e) => setNewRunCohortValue(e.currentTarget.value)}
                    />
                  )}
                  <Select
                    size="xs"
                    ...
                    value={newRunInterventionType}
                    onChange={setNewRunInterventionType}
                  />
                </Group>
                <TextInput
                  size="xs"
                  ...
                  onChange={(e) => setNewRunDescription(e.currentTarget.value)}
                />
                <Button size="xs" variant="filled" onClick={handleCreateRun} disabled={!newRunName.trim()}>
                  Create Run
                </Button>
              </Stack>
            </Card>
          ...
          {improvementRuns.length === 0 && !showCreateRun ? (
            <Text size="sm" c="dimmed">
              No improvement runs yet. Create one to start measuring quality lift.
            </Text>
          ) : (
            <Stack gap="sm">
                ...
                <Card key={run.id} withBorder radius="sm" p="sm" className="ag-card-tier-2">
                  <Group justify="space-between" mb="xs">
                    <Group gap="xs">
                      ...
                      <Badge color={stateColor(run.status)} variant="light" size="sm">
                        {run.status}
                      </Badge>
                      {run.promotionDecision && run.promotionDecision !== "pending" && (
                        <Badge color={stateColor(run.promotionDecision)} variant="dot" size="sm">
                  ...
                  <Group gap="lg" mb="xs">
                    <Text size="xs">
                      Cohort: <strong>{run.cohort.selectionMethod}</strong> ({run.cohort.resolvedCount ?? 0} specs)
                    </Text>
                    <Text size="xs">
                      Intervention: <strong>{run.intervention.type}</strong>
                    </Text>
                  </Group>
                  {run.metrics?.delta && (
                    <Group gap="lg" mb="xs">
                      <Text size="xs">
                        Quality Lift: <strong>{run.metrics.delta.qualityLiftPct?.toFixed(1) ?? "—"}%</strong>
                      </Text>
                  ...
                  {run.metrics?.before && run.metrics?.after && (
                    <Group gap="lg" mb="xs">
                      <Text size="xs" c="dimmed">
                        Before: {pct(run.metrics.before.firstPassApprovalRate)} → After:{" "}
                        {pct(run.metrics.after.firstPassApprovalRate)}
                      </Text>
                    </Group>
                  )}
                  <Group gap="xs">
                    {run.status === "draft" && (
                      <Button size="xs" variant="light" color="blue" onClick={() => handleStartRun(run.id)}>
                        Start
                      </Button>
                    )}
                    {run.status === "running" && (
                      <Button size="xs" variant="light" color="green" onClick={() => handleCompleteRun(run.id)}>
                        Complete
                      </Button>
                    )}
                    {run.status === "completed" && run.promotionDecision === "pending" && (
                      <>
                        <Button size="xs" variant="light" color="green" onClick={() => handlePromoteRun(run.id)}>
                          Promote
                        </Button>
                        <Button size="xs" variant="light" color="orange" onClick={() => handleRollbackRun(run.id)}>
                          Rollback
                        </Button>
                      </>
                    )}
      ...
      {tab === "breakers" && (
        <Card withBorder radius="md" p="md" className="ag-card-tier-1">
          ...
          {breakers.length === 0 ? (
            <Text size="sm" c="dimmed">
              No circuit breakers configured. They auto-create when automation rules fire.
            </Text>
          ) : (
            <ScrollArea>
              <Table className="ag-trend-table" striped highlightOnHover>
                  ...
                  {breakers.map((b) => {
                    ...
                    return (
                      <Table.Tr key={b.id}>
                        ...
                        <Table.Td>
                          <Badge color={stateColor(b.state)} variant="filled" size="sm">
                            {b.state}
                          </Badge>
                        </Table.Td>
                        ...
                        <Table.Td>
                          <Text size="xs">
                            trips: {b.stats?.totalTrips ?? 0}, blocked: {b.stats?.blockedTriggers ?? 0}
                          </Text>
                        </Table.Td>
                        <Table.Td>
                          {b.state !== "closed" && (
                            <Button size="xs" variant="light" color="green" onClick={() => handleResetBreaker(b.id)}>
                              Reset
                            </Button>
                          )}
                        </Table.Td>
        ...
        <Card withBorder radius="md" p="md" className="ag-card-tier-1">
          ...
          <Group gap="sm" mb="md">
            <Select
              ...
              searchable
            />
            <Button size="xs" variant="filled" disabled={!backtestRuleId} onClick={handleBacktest}>
              Run Backtest
            </Button>
          </Group>
          {backtestReport && (
            <Card withBorder p="sm" radius="sm" className="ag-card-tier-2">
              <Text fw={600} mb="xs">
                {backtestReport.ruleName}
              </Text>
              <Group gap="lg" mb="xs">
                <Text size="sm">
                  Events scanned: <strong>{backtestReport.totalEventsScanned}</strong>
                </Text>
                <Text size="sm">
                  Matched: <strong>{backtestReport.matchedEvents}</strong>
                </Text>
                <Text size="sm">
                  Est. jobs: <strong>{backtestReport.estimatedJobsEnqueued}</strong>
                </Text>
              </Group>
              <Group gap="lg" mb="xs">
                <Text size="sm">
                  Peak velocity: <strong>{backtestReport.peakTriggersPerMinute.toFixed(1)}/min</strong>
                </Text>
                <Text size="sm">
                  Avg: <strong>{backtestReport.avgTriggersPerHour.toFixed(1)}/hr</strong>
                </Text>
              </Group>
      ...
      {tab === "gaps" && expertMode && (
        <Card withBorder radius="md" p="md" className="ag-card-tier-1">
          <Group justify="space-between" mb="sm">
            ...
            <Button size="xs" variant="light" onClick={handleGapAnalysis}>
              Run Analysis
            </Button>
          </Group>
          {gapReport && (
            <>
              <Group gap="lg" mb="md">
                <Text size="sm">
                  Validator passes: <strong>{gapReport.totalValidatorPasses}</strong>
                </Text>
                <Text size="sm">
                  Human-rejected after pass: <strong>{gapReport.humanRejectedAfterPass}</strong>
                </Text>
                <Text size="sm">
                  Gap rate: <strong>{pct(gapReport.gapRate)}</strong>
                </Text>
              </Group>
